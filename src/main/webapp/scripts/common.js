 ...
 */
  ...
  function dataDateToFrontend(date,lang){
    ...
    if(lang=='EN'||lang=='EN_US'){
      ...
      if(dataDate.getMonth() == 1 && dataDate.getDate() == 29){
        dateresult = new Date(dataDate.setFullYear(dataDate.getFullYear())).format('dd/mm/yyyy');
        dateresult = dateresult.split('/');
        dateresult = dateresult[0] + '/' + dateresult[1] + '/' + (Number(dateresult[2])+543);
      } else {
        dateresult = new Date(dataDate.setFullYear(dataDate.getFullYear()+543)).format('dd/mm/yyyy');
      }
    }
...
//Alert Modal
var AlertUtil={};
AlertUtil.oKOnly=function (message,func) {
    bootbox.dialog({
        message: " ",
        title: "<div class='page-header'><center><h4>" + message + "</h4></center></div>",
        buttons: {
            success: {
                label: "<div class='text-center'>" + CommonLabel.BUTTON_OK + "</div>",
                className: "btn-primary",
                callback: func
            }
        },
